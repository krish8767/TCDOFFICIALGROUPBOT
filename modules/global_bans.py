...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in SUPPORT_USERS:
        message.reply_text("OOOH someone's trying to gban a support user! *grabs D$CK*")
        return
    ...
    if sql.is_user_gbanned(user_id):
        if not reason:
            message.reply_text("This BITC# is already gbanned; I'd change the reason, but you haven't given me one...")
            return

        old_reason = sql.update_gban_reason(user_id, user_chat.username or user_chat.first_name, reason)
        if old_reason:
            message.reply_text("This BITC# is already gbanned, for the following reason:\n"
                               "<code>{}</code>\n"
                               "I've gone and updated it with your new reason!".format(html.escape(old_reason)),
                               parse_mode=ParseMode.HTML)
        else:
            message.reply_text("This BITC# is already gbanned, but had no reason set; I've gone and updated it!")
