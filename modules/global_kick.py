...
def gkick(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in SUDO_USERS or int(user_id) in SUPPORT_USERS:
        message.reply_text("OHHH! Someone's trying to gkick a sudo/support user! *Grabs D$CK*")
        return
    if int(user_id) == OWNER_ID:
        message.reply_text("Wow! Someone's so noob that he want to gkick my owner! *Grabs BIG BLACK D$CK*")
        return
    if int(user_id) == bot.id:
        message.reply_text("OHH... Let me kick myself.. No way you BITC# ")
        return
    chats = get_all_chats()
    message.reply_text("Globally kicking BITC# @{}".format(user_chat.username))
    for chat in chats:
        try:
