...
def gmute(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in SUDO_USERS:
        message.reply_text("I spy, with my little eye... a sudo user war! Why are you guys trying to F##K each other?")
        return

    if int(user_id) in SUPPORT_USERS:
        message.reply_text("OOOH someone's trying to gmute a support user! *grabs D$CK*")
        return

    if user_id == bot.id:
        message.reply_text("-_- So funny, lets gmute myself why don't I? Nice try you BITC#.")
        return
    ...
    if sql.is_user_gmuted(user_id):
        if not reason:
            message.reply_text("This BITC# is already gmuted; I'd change the reason, but you haven't given me one...")
            return

        success = sql.update_gmute_reason(user_id, user_chat.username or user_chat.first_name, reason)
        if success:
            message.reply_text("This BITC# is already gmuted; I've gone and updated the gmute reason though!")
        else:
